...
# ================= НАСТРОЙКИ =================
BOT_TOKEN = "8945721473:AAHhI3KxtG2w364Z8b01qjk8Nmy18a4vRSM"
OWNER_USERNAME = "NorikAmiri"
SECRET_CODE = "norik228"
...
async def init_db():
    ...
    async with DB_POOL.acquire() as conn:
        ...
        # ================= ОДНОРАЗОВАЯ ОЧИСТКА БАЗЫ =================
        flag = await conn.fetchrow("SELECT value FROM settings WHERE key='db_cleared_v3'")
        if not flag:
            await conn.execute(
                "TRUNCATE deals, business_connections, business_peers, "
                "send_log, banned, admins, users RESTART IDENTITY"
            )
            await conn.execute(
                "DELETE FROM settings WHERE key IN ('business_connection_id', 'bot_avatar_url')"
            )
            await conn.execute(
                "INSERT INTO settings (key, value) VALUES ('db_cleared_v3', '1')"
            )
            print("🧹 База очищена (одноразово)")
        # =============================================================
...
async def start(message: Message):
    ...
    # 👑 Гарантированно добавляем главного владельца в таблицу admins,
    # чтобы он тоже получал уведомления об оплатах.
...
async def process_successful_payment(message: Message):
    ...
    # ============ СООБЩЕНИЕ ПОКУПАТЕЛЮ (с премиум-эмодзи) ============
    try:
        await message.answer(
    ...
    # Главный владелец (OWNER_USERNAME) гарантированно получает уведомление.
    # Если он уже в admins — попадёт через admin_ids, если нет — добавим из users по username.
    try:
        async with DB_POOL.acquire() as conn:
        ...
        if owner_row:
            recipients.add(owner_row["user_id"])
    except Exception as e:
        print(f"[payment] Ошибка поиска владельца: {e}")

    # BAN_MANAGER всегда в списке
    if BAN_MANAGER_ID:
        recipients.add(BAN_MANAGER_ID)

    if deal_owner_id:
        recipients.add(deal_owner_id)

    # 👑 Покупателя НЕ удаляем из получателей, если он сам админ / владелец /
    # BAN_MANAGER — он тоже получит уведомление о новой оплате.
    buyer_is_admin = (
        (buyer_id in admin_ids)
        or (buyer_id == BAN_MANAGER_ID)
        or (buyer_id == deal_owner_id)
    )
